...
class BaseCrudController : KoinComponent {
    ...
    fun findPage(name: String, params: IUmiPaginationParams): String {
        return when (name) {
            ...
            Name_constant -> MySerializeJson.encodeToString(
                DataBox.ok(
                    service.findPage(
                        Meta.constant,
                        params.toSqlPagination()
            ...
            Name_operator -> MySerializeJson.encodeToString(
                DataBox.ok(
                    service.findPage(
                        Meta.operator,
                        params.toSqlPagination()
            ...
            Name_expression -> MySerializeJson.encodeToString(
                DataBox.ok(
                    service.findPage(
                        Meta.expression,
                        params.toSqlPagination()
            ...
            Name_rule -> MySerializeJson.encodeToString(
                DataBox.ok(
                    service.findPage(Meta.rule, params.toSqlPagination()).map { it.toRuleCommon(service) })
            )

            Name_ruleGroup -> MySerializeJson.encodeToString(
                DataBox.ok(
                    service.findPage(
                        Meta.ruleGroup,
                        params.toSqlPagination()
                    ).map { it.toRuleCommon(service) })
            )

            Name_action -> MySerializeJson.encodeToString(
                DataBox.ok(
                    service.findPage(
                        Meta.ruleAction,
                        params.toSqlPagination()
            ...
            MySerializeJson.encodeToString(
                DataBox.ok(
                    service.save(
                        ...
                        e,
                        e.id == null,
                        e.id?.let { "paramType/${it}" }).apply { toBean(service) })
            )
        }
            ...
            MySerializeJson.encodeToString(
                DataBox.ok(
                    service.save(
                        ...
                        e,
                        e.id == null,
                        e.id?.let { "constant/${it}" }).apply { toBean(service) })
            )
        }
            ...
            MySerializeJson.encodeToString(
                DataBox.ok(
                    service.save(
                        ...
                        e,
                        e.id == null,
                        e.id?.let { "expression/${it}" }).apply { toBean(service) })
            )
        }
    ...
    private fun batchDel(name: String, ids: List<Int>): Long {
        val count = when (name) {
            Name_domain -> service.delete(
                Meta.domain,
                { Meta.domain.id inList ids.map { it } },
                null,
                ids.map { "domain/$it" })

            Name_param -> service.delete(
                Meta.param,
            ...
            Name_paramType -> service.delete(
                ...
                null,
                ids.map { "paramType/$it" })

            Name_constant -> service.delete(
                Meta.constant,
                { Meta.constant.id inList ids.map { it } },
                null,
                ids.map { "constant/$it" })

            Name_operator -> service.delete(
                Meta.operator,
                { Meta.operator.id inList ids.map { it } },
                null,
                ids.map { "operator/$it" })

            Name_expression -> service.delete(
                Meta.expression,
                { Meta.expression.id inList ids.map { it } },
                null,
                ids.map { "expression/$it" })
    ...
    fun initDictDataInDb(): String {
        ...
        val list = listOf(
            ParamType(
                StringType.label,
                StringType.code,
                StringType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                true
            ),
            ParamType(
                IntType.label,
                IntType.code,
                IntType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                true
            ),
            ParamType(
                LongType.label,
                LongType.code,
                LongType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                true
            ),
            ParamType(
                DoubleType.label,
                DoubleType.code,
                DoubleType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                true
            ),
            ParamType(
                DateTimeType.label,
                DateTimeType.code,
                DateTimeType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                true
            ),
            ParamType(
                BoolType.label,
                BoolType.code,
                BoolType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                true
            ),
            ParamType(
                StringSetType.label,
                StringSetType.code,
                StringSetType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                false
            ),
            ParamType(
                IntSetType.label,
                IntSetType.code,
                IntSetType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                false
            ),
            ParamType(
                LongSetType.label,
                LongSetType.code,
                LongSetType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                false
            ),
            ParamType(
                DoubleSetType.label,
                DoubleSetType.code,
                DoubleSetType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                false
            ),
            ParamType(
                DateTimeSetType.label,
                DateTimeSetType.code,
                DateTimeSetType.supportOperators().mapNotNull { map[it]?.id }.joinToString(","),
                false
            ),
        )
