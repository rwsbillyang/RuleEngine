...
/**
 * @param edit: 若提供的是ProFormLayoutType('ModalForm' | 'DrawerForm' )，则按当前配置进入编辑页面；
 ...
 */
export interface EditConfig<T extends BaseRecord> {
  edit: 'ModalForm' | 'DrawerForm' | ((e?: Partial<T>) => string | 'ModalForm' | 'DrawerForm' | undefined)
...
export interface MyProTableProps<T extends BaseRecord, Q extends BasePageQuery> extends Omit<ProTableProps<T, Q, 'text'>, 'params' | 'request' | 'dataSource'> {
  ...
  editConfig?: EditConfig<T> //不提供则不进入编辑页面，
